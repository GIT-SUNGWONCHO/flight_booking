...
Invoke-Stage "[1/12] 라벨 판정 유닛테스트" { node test/test_autoconfirm.js } "유닛테스트 실패"

Invoke-Stage "[2/12] 유틸(날짜 자동감지/접두어 매칭) 유닛테스트" { node test/test_util.js } "유틸 유닛테스트 실패"

Invoke-Stage "[3/12] 유저스크립트 빌드" {
  node build.mjs
  node --check userscript/ke-award-macro.user.js
} "빌드 실패"

Reset-Browsers
Invoke-Stage "[4/12] 브라우저 통합테스트" {
  & ./.venv/Scripts/python.exe test/test_integration.py
} "통합테스트 실패"

Reset-Browsers
Invoke-Stage "[5/12] 유저스크립트(HUD) 테스트" {
  & ./.venv/Scripts/python.exe test/test_hud.py
} "HUD 테스트 실패"

Reset-Browsers
Invoke-Stage "[6/12] 녹화/재생 테스트" {
  & ./.venv/Scripts/python.exe test/test_recorder.py
} "녹화/재생 테스트 실패"

Reset-Browsers
Invoke-Stage "[7/12] 단계 편집 테스트" {
  & ./.venv/Scripts/python.exe test/test_editor.py
} "편집 테스트 실패"

Reset-Browsers
Invoke-Stage "[8/12] 단계 우선순위 테스트" {
  & ./.venv/Scripts/python.exe test/test_precedence.py
} "우선순위 테스트 실패"

Reset-Browsers
Invoke-Stage "[9/12] 건너뜀 보고 / 추측클릭 제거 확인" {
  & ./.venv/Scripts/python.exe test/test_skipreport.py
} "건너뜀 보고 테스트 실패"

Reset-Browsers
Invoke-Stage "[10/12] 헛클릭 감지·재시도 테스트" {
  & ./.venv/Scripts/python.exe test/test_deadclick.py
} "헛클릭 재시도 테스트 실패"

Reset-Browsers
Invoke-Stage "[11/12] 모달 가림 테스트" {
  & ./.venv/Scripts/python.exe test/test_modalblock.py
} "모달 가림 테스트 실패"

Reset-Browsers
Invoke-Stage "[12/12] 스크롤 팝업 테스트" {
