...
Invoke-Stage "[1/31] 라벨 판정 유닛테스트" { node test/test_autoconfirm.js } "유닛테스트 실패"

Invoke-Stage "[2/31] 유틸(날짜 자동감지/접두어 매칭) 유닛테스트" { node test/test_util.js } "유틸 유닛테스트 실패"

Invoke-Stage "[3/31] 유저스크립트 빌드" {
  node build.mjs
  node --check userscript/ke-award-macro.user.js
} "빌드 실패"

Reset-Browsers
Invoke-Stage "[4/31] 브라우저 통합테스트" {
  & ./.venv/Scripts/python.exe test/test_integration.py
} "통합테스트 실패"

Reset-Browsers
Invoke-Stage "[5/31] 유저스크립트(HUD) 테스트" {
  & ./.venv/Scripts/python.exe test/test_hud.py
} "HUD 테스트 실패"

Reset-Browsers
Invoke-Stage "[6/31] 녹화/재생 테스트" {
  & ./.venv/Scripts/python.exe test/test_recorder.py
} "녹화/재생 테스트 실패"

Reset-Browsers
Invoke-Stage "[7/31] 단계 편집 테스트" {
  & ./.venv/Scripts/python.exe test/test_editor.py
} "편집 테스트 실패"

Reset-Browsers
Invoke-Stage "[8/31] 단계 우선순위 테스트" {
  & ./.venv/Scripts/python.exe test/test_precedence.py
} "우선순위 테스트 실패"

Reset-Browsers
Invoke-Stage "[9/31] 건너뜀 보고 / 추측클릭 제거 확인" {
  & ./.venv/Scripts/python.exe test/test_skipreport.py
} "건너뜀 보고 테스트 실패"

Reset-Browsers
Invoke-Stage "[10/31] 좌석 소진 감지 테스트" {
  & ./.venv/Scripts/python.exe test/test_seatgone.py
} "좌석 소진 감지 테스트 실패"

Reset-Browsers
Invoke-Stage "[11/31] 헛클릭 감지·재시도 테스트" {
  & ./.venv/Scripts/python.exe test/test_deadclick.py
} "헛클릭 재시도 테스트 실패"

Reset-Browsers
Invoke-Stage "[12/31] 모달 가림 테스트" {
  & ./.venv/Scripts/python.exe test/test_modalblock.py
} "모달 가림 테스트 실패"

Reset-Browsers
Invoke-Stage "[13/31] 스크롤 팝업 테스트" {
  & ./.venv/Scripts/python.exe test/test_scrollmodal.py
} "스크롤 팝업 테스트 실패"

Reset-Browsers
Invoke-Stage "[14/31] 중복 동의 테스트" {
  & ./.venv/Scripts/python.exe test/test_doubleagree.py
} "중복 동의 테스트 실패"

Reset-Browsers
Invoke-Stage "[15/31] 동의 2개 모달 테스트" {
  & ./.venv/Scripts/python.exe test/test_twoagree.py
} "동의 2개 테스트 실패"

Reset-Browsers
Invoke-Stage "[16/31] 셀렉터 집기 / 패널 드래그 테스트" {
  & ./.venv/Scripts/python.exe test/test_picker.py
} "집기/드래그 테스트 실패"

Reset-Browsers
Invoke-Stage "[17/31] 건너뛰기 금지 테스트" {
  & ./.venv/Scripts/python.exe test/test_noskip.py
} "건너뛰기 금지 테스트 실패"

Reset-Browsers
Invoke-Stage "[18/31] 무장 유지 / 결제창 판정 테스트" {
  & ./.venv/Scripts/python.exe test/test_armpersist.py
} "무장 유지 테스트 실패"

Reset-Browsers
Invoke-Stage "[19/31] 달력 최신날짜 / 목표날짜 형식 테스트" {
  & ./.venv/Scripts/python.exe test/test_calendar.py
} "달력 테스트 실패"

Reset-Browsers
Invoke-Stage "[20/31] 통화 KRW / 결제수단 대체 테스트" {
  & ./.venv/Scripts/python.exe test/test_currency.py
} "통화/결제수단 테스트 실패"

Reset-Browsers
Invoke-Stage "[21/31] 통화 변경 후 화면 되돌아감 테스트" {
  & ./.venv/Scripts/python.exe test/test_currestart.py
} "통화 재시작 테스트 실패"

Reset-Browsers
Invoke-Stage "[22/31] 발사 시각 입력 테스트" {
  & ./.venv/Scripts/python.exe test/test_opentime.py
} "발사 시각 테스트 실패"

Reset-Browsers
Invoke-Stage "[23/31] 목표 날짜 오픈 대기 테스트" {
  & ./.venv/Scripts/python.exe test/test_openwait.py
} "목표 날짜 대기 테스트 실패"

Reset-Browsers
Invoke-Stage "[24/31] 로그인 확인 테스트" {
  & ./.venv/Scripts/python.exe test/test_login.py
} "로그인 확인 테스트 실패"

Invoke-Stage "[25/31] 바로 시작 주소 테스트" {
  & node test/test_deeplink.js
} "바로 시작 주소 테스트 실패"

Invoke-Stage "[26/31] 조회 응답 계측 테스트" {
  & node test/test_probe.js
} "조회 응답 계측 테스트 실패"

Reset-Browsers
Invoke-Stage "[27/31] 달력 건너뛰기 테스트" {
  & ./.venv/Scripts/python.exe test/test_skipcal.py
} "달력 건너뛰기 테스트 실패"

Reset-Browsers
Invoke-Stage "[28/31] 단계 원인 분류 테스트" {
  & ./.venv/Scripts/python.exe test/test_stepwhy.py
} "단계 원인 분류 테스트 실패"

Reset-Browsers
Invoke-Stage "[29/31] 좁은 창 안내 테스트" {
  & ./.venv/Scripts/python.exe test/test_narrow.py
} "좁은 창 안내 테스트 실패"

Reset-Browsers
Invoke-Stage "[30/31] 시작 경로 일치 테스트" {
  & ./.venv/Scripts/python.exe test/test_parity.py
} "시작 경로 일치 테스트 실패"
